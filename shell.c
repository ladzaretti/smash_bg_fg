                      ...
                      int *argcAllocedPtr)
{
    ...
    if (argc > 1)
    { /* cmd->globResult is already initialized */
        flags = GLOB_APPEND;
               ...
               int inBg)
{
    ...
    {
        job = jobList->head; /*set job to the head of the bg jobs list*/
        /*Scan the job list and print jobs' status*/
        /*if job->runningProgs is zero, the job is removed. no need to check.*/
        while (job)
        {
    ...
    {
        int argc = 0;
        char *temp_text = newJob.text, *N_remaining_str = NULL;
        // First of all do some syntax checking.
        // If the syntax check fails return 1
        /*count number of arguments for fg/bg -> argc*/
        while (*temp_text)
            if (isspace(*(temp_text++)) && (*temp_text) && (!isspace(*temp_text))) /*find sequences where there is non space char after a space -> indicating argument*/
                argc++;
        if (argc != 1)
        {
        ...
        /*get [N]*/
        temp_text = &(newJob.progs[0].argv[1][1]);                     /*set temp_text ptr after the % sign*/
        jobNum = (int)strtod(temp_text, &N_remaining_str);             /*convert to int via casting from double*/
        if ((temp_text == N_remaining_str) || (*N_remaining_str != 0)) /*if extracted none or if there are leftovers->error*/
        {
            fprintf(stderr, "%s: bad argument '%s'\n", newJob.progs[0].argv[0], newJob.progs[0].argv[1]);
            return 1;
        }
        ...
        if (!strcmp(newJob.progs[0].argv[0], "fg"))
        {
            if (!tcsetpgrp(STDIN_FILENO, job->pgrp)) /*returns 0 if successful, -1 on failure*/
                jobList->fg = job;
            else
            {
        ...
        if (kill(job->pgrp, SIGCONT)) /*returns 0 if successful, -1 on failure*/
        {
            perror("kill error");
            return 1;
        }
